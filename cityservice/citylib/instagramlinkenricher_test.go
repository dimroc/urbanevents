...
func TestInstagramLinkEnrich(t *testing.T) {
	Convey("Given a GeoEvent with an instagram shortcode url", t, func() {
	...
	Convey("Given a GeoEvent with an instagram longer url", t, func() {
		geoevent := Fixture.GeoEvents[0]
		geoevent.ExpandedUrl = "https://instagram.com/p/6CtKxPEPMTbhmVUeFMBJcwCh7wnxOn4peEVGU0/"
		...
		media := &Fixture.GetInstagramMedia()[0]
		mediaRetriever := mock_citylib.NewMockMediaRetriever(mockCtrl)
		mediaRetriever.EXPECT().GetShortcode("6CtKxPEPMTbhmVUeFMBJcwCh7wnxOn4peEVGU0").Return(media, nil)
